...
//add new Column type to Datatable to Allow Search in input
/*$.fn.dataTable.ext.type.search['input'] = function(value) {
...
function FilterDatatable(table) {
    ...
    $(table.table().header()).find( '.searchableTh' ).each(function(index, value){
        ...
        if($(this).find("i.fas.fa-search").length){
...
function dataTablizer(table){
    var dom             = table.data('dom'),
        ...
        input           = table.data('input');//input column Class
    ...
    var tabelizedTable = $(table).DataTable({
        ...
        info            : false,
        "autoWidth": false,
        'columnDefs': [ {'targets': nonOrderable, 'orderable': false},
                        {"targets": input, "type": "input"}
        ]
    });
...
$(document).ready(function() {
    ...
    //update Search Value for Input Cells
    $('.right-content').on( 'change', 'td.input-cell input, td.input-cell select , td.input-cell textarea ', function () {
        var input   = $(this);
...
function getInputCellValue(cell){
    var value   = "";

    //Start Getting Value
    var text        = cell.children("input[type='text']");
...
function updateCellValue(cell,value){
    ...
    cell.attr('data-search' , value).attr('data-sort' , value);
    table.cell(cell).invalidate('dom').draw();
    table.draw('page');
